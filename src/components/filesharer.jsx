	...
	render: function() {
		...
		if(this.state.initialized){
			result = (
				<div>
					<div>
            <span>{this.props.opts.my_id_label || 'Meu ID:'} </span>
		...
		return (
			<div>
				...
				<div className="mui-textfield">
					...
					<label>{this.props.opts.peer_id_label || 'ID Peer'}</label>
				</div>
				<button className="mui-btn mui-btn--accent" onClick={this.connect}>
					{this.props.opts.connect_label || 'CONECTAR'}
				</button>
			</div>
		);
	},
		...
		return (
			<div id="file_list">
      			<table className="mui-table mui-table--bordered">
					<thead>
					  <tr>
					    <th>{this.props.opts.file_list_label || 'Compartilhados com você: '}</th>
					  </tr>
					</thead>
					<tbody>
	...
	renderNoFiles: function () {
		return (
			<span id="no_files_message">
				{this.props.opts.no_files_label || 'O usuário não compartilhou arquivos'}
			</span>
		);
	},
